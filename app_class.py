...
class App: # we will be defining a class 'App'
    def __init__(self):
        pygame.init()
        self.window = pygame.display.set_mode((WIDTH, HEIGHT)) # window of given height and width
        self.running = True
        ...
        self.lockedCells = []
        self.incorrectCells = []
        self.font = pygame.font.SysFont("arial", cellSize//2) # font for the sudoku grid
        self.grid = []
        self.getPuzzle("4")
        self.load()
    ...
    def playing_events(self): # playing the game
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN: # when user clicks
                selected = self.mouseOnGrid() # when clicked
                if selected:
            ...
            if event.type == pygame.KEYDOWN: # user pressing key
    ...
    def playing_update(self): # updating the game
...
# check the board
    ...
    def checkRows(self):
        for yidx, row in enumerate(self.grid):
            possibles = [1,2,3,4,5,6,7,8,9] #only integer allowed
            for xidx in range(9):
                if self.grid[yidx][xidx] in possibles:
                    possibles.remove(self.grid[yidx][xidx])
                else:
...
#helper functions
    def getPuzzle(self, difficulty):
        html_doc = requests.get("https://nine.websudoku.com/?level={}".format(difficulty)).content # scraping the website
        soup = BeautifulSoup(html_doc,features="html.parser") # searching for the puzzle
        ...
        'f85', 'f86', 'f87', 'f88'] #only the cell values are picked out
        data = []
        for cid in ids:
    ...
    def shadeIncorrectCells(self, window, incorrect): # when the entered value is wrong
    ...
    def shadeLockedCells(self, window, locked): #for the values that are part of the puzzle and should not be changed; locked
        for cell in locked:
            pygame.draw.rect(window, LOCKEDCELLCOLOUR, (cell[0]*cellSize+gridPos[0], cell[1]*cellSize+gridPos[1], cellSize, cellSize))

    def drawNumbers(self, window): #input values into the cell
        for yidx, row in enumerate(self.grid):
    ...
    def drawSelection(self, window, pos): # selecting a cell
        pygame.draw.rect(window, LIGHTBLUE, ((pos[0]*cellSize)+gridPos[0], (pos[1]*cellSize)+gridPos[1], cellSize, cellSize))

    def drawGrid(self, window):
        pygame.draw.rect(window, BLACK, (gridPos[0], gridPos[1], WIDTH-150, HEIGHT-150), 2) #drawing inside the window with margins
        for x in range(9):
            # differentiating smaller 3x3 grids
    ...
    def mouseOnGrid(self): #tracking the mouse position to find whether or not the mouse is on the grid
    ...
    def load(self):
        ...
        # setting the locked cells using original board
        for yidx, row in enumerate(self.grid):
            for xidx, num in enumerate(row):
                if num != 0:
                    self.lockedCells.append([xidx, yidx])

    def isInt(self, string):# ensuring only integers are entered
        try:
            int(string)
            return True
        except:
